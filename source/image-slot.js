...
(() => {
  ...
  const STATE_FILE = (() => {
    try {
      ...
      const base = f.replace(/\.dc\.html$|\.html?$/i, '');
      if (base) return base + '.image-slots.state.json';
    } catch (e) {}
  ...
  // ── Host hook ───────────────────────────────────────────────────────────
  // The store is deliberately private: a page that only ever fills slots by
  // hand has no reason to reach it. K10 does — it carries a system's build-up
  // drawing from one specification to the next, so it needs to read one slot
  // and write another. Expose exactly that much. setSlot already notifies every
  // mounted element and persists, so a write lands live; ready() resolves once
  // the sidecar has been merged, which is the only safe moment to conclude a
  // slot is genuinely empty rather than simply not loaded yet.
  window.ImageSlots = {
  ...
  // Clipboard paste: fill the slot under the pointer (or the last one the
  // user pointed at / clicked) from a snipped screenshot on Ctrl/Cmd+V.
  let pasteTarget = null;
  const trackTarget = (e) => {
    const path = e.composedPath ? e.composedPath() : [e.target];
    for (const n of path) {
      if (n && n.tagName === 'IMAGE-SLOT') { pasteTarget = n; return; }
    }
  };
  document.addEventListener('pointerover', trackTarget, true);
  document.addEventListener('pointerdown', trackTarget, true);
  document.addEventListener('paste', (e) => {
    const target = pasteTarget;
    if (!target || !target.isConnected || !target._ingest) return;
    const dt = e.clipboardData;
    const items = (dt && dt.items) || [];
    for (const it of items) {
      if (it.kind === 'file' && it.type && it.type.indexOf('image/') === 0) {
        const f = it.getAsFile();
        if (f) { e.preventDefault(); target._ingest(f); }
        return;
      }
    }
